...
const EditVideoGames = () => {
    ...
    useEffect(() => {
        setLoading(true);
        axios.get(`http://localhost:5555/videogames/${id}`)
        .then((response) => {
    ...
    const handleEditVideoGame = () => {
        ...
        axios
            ...
            .then(() => {
                setLoading(false);
                enqueueSnackbar('Video Game Edited Successfully', {variant: 'success'});
                navigate('/');
            })
            .catch((error) => {
                setLoading(false);
    ...
    return (
        <div className='p-4'>
            <BackButton />
            <h1 className='text-3x1 my-4'>Edit Video Game</h1>
            {loading ? <Spinner /> : ''}
            <div className='flex flex-col border-2 border-sky-400 roundedd-x1 w-[600px] p-4 mx-auto'>
                ...
                <button className='p-2 bg-sky-300 m-8' onClick={handleEditVideoGame}>
                    Save
                </button>
            </div>
